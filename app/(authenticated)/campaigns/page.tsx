    ...
    <main className="flex min-h-screen flex-col items-center gap-6 p-24">
      ...
      <div className="flex w-full max-w-2xl items-center justify-between">
        ...
        <div className="flex gap-2">
          ...
          <Link href="/campaigns/new" className="rounded bg-accent px-4 py-2 text-sm text-accent-foreground">
            + New campaign
          </Link>
        </div>
      </div>
      ...
      <div className="flex w-full max-w-2xl flex-col gap-2">
        {(gmCampaigns ?? []).length === 0 && memberCampaigns.length === 0 ? (
          <p className="text-sm text-muted">You&apos;re not part of any campaigns yet.</p>
        ) : (
          <>
            {(gmCampaigns ?? []).map((c) => (
