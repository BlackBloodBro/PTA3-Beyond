...
// and data-fetching, just under a campaign-scoped path for a Wild/pool Pokemon. See
// [[Give Wild Pokemon their own campaign-scoped page]]. Trainer-related fields/links throughout this
// file are unreachable in practice (the guard below ensures this page only ever renders for a
// Pokemon with no owning Trainer) but kept as-is to stay a faithful mirror of the shared page.
export default async function WildPokemonPage({
  params,
  searchParams,
}: {
  ...
  // A Trainer-owned Pokemon (or a pool Pokemon tagged to a different campaign than the URL claims)
  // doesn't resolve here -- this namespace is exclusively for this campaign's own Wild Pokemon.
  // Checked via the raw `pokemon.trainers_pokemon` (not the manually-cast `ownerLink`) so TS's
  // aliased-condition narrowing has no binding back to `ownerLink` -- it stays its declared
  // `{...} | null` type for every `ownerLink?.` access below instead of collapsing to `never`.
  if (pokemon.trainers_pokemon || poolCampaign?.id !== campaignId) {
    notFound()
  }

  const basePath = pokemonHref({ id: pokemonId, hasOwner: false, campaignId })
  ...
  return (
    <main className="flex min-h-screen flex-col items-center gap-6 p-24">
      ...
      <div className="flex w-full max-w-6xl items-center gap-3">
        <Link href={`/campaigns/${campaignId}/wild-pokemon`} className="text-sm underline">
          ← Wild Pokémon
        </Link>
        {trainerId && (
          <>
