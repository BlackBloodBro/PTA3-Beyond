...
export default async function PokemonPage({
  ...
  const { data: pokemon } = await supabase
    ...
    .select(
      ...
      trainers_pokemon(
        ...
        trainers(name, user_id, campaigns(id, name, gm_user_id)),
  ...
  const ownerLink = pokemon.trainers_pokemon as unknown as {
    ...
    trainers: { name: string; user_id: string; campaigns: { id: string; name: string; gm_user_id: string } | null } | null
        ...
        <aside className="w-64 shrink-0">
          <section className="rounded border p-4">
            <div className="mb-3 flex flex-col items-center gap-1 text-center">
              ...
              {pokemon.is_shiny && <span className="text-xs font-medium text-amber-600">✦ Shiny</span>}
              {!isOwner && <span className="text-xs text-neutral-500">(GM view)</span>}
              <LevelLine />
            </div>
            ...
            <div className="mb-2 flex items-center justify-between border-t pt-3">
              ...
              {canEditInfo && !isEditingInfo && (
                <Link
                  href={`/pokemon/${pokemonId}?editInfo=1`}
            ...
            {canEditInfo && isEditingInfo ? (
              ...
              {isGM && (
                <>
                  ...
                  <select id="loyaltyId" name="loyaltyId" className="rounded border p-2" defaultValue={pokemon.loyalty_id ?? ''}>
                    <option value="">—</option>
                    {(allLoyalties ?? []).map((l) => (
                  ...
                  <select id="natureId" name="natureId" className="rounded border p-2" defaultValue={pokemon.nature_id ?? ''}>
                  ...
                  <select id="weightId" name="weightId" className="rounded border p-2" defaultValue={pokemon.weight_id ?? ''}>
                  ...
                  <select id="sizeId" name="sizeId" className="rounded border p-2" defaultValue={pokemon.size_id ?? ''}>
                  ...
                  <select id="gender" name="gender" className="rounded border p-2" defaultValue={pokemon.gender ?? ''}>
                  ...
                  <select id="heldItemId" name="heldItemId" className="rounded border p-2" defaultValue={pokemon.held_item_id ?? ''}>
                    <option value="">None</option>
                    {(allItems ?? []).map((it) => (
                  ...
                  <select id="type1Id" name="type1Id" className="rounded border p-2" defaultValue={pokemon.type_1_id ?? ''}>
                  ...
                  <select id="type2Id" name="type2Id" className="rounded border p-2" defaultValue={pokemon.type_2_id ?? ''}>
