...
import { convertTrainerToNpc } from '@/app/campaigns/[id]/actions'
  ...
  return (
    <main className="flex min-h-screen flex-col items-center gap-6 p-24">
      ...
      <div className="flex w-full max-w-2xl items-center justify-between">
        ...
        <Link href={`/campaigns/${id}/npcs/new`} className="rounded bg-black px-4 py-2 text-sm text-white">
          + New NPC
        </Link>
      </div>
      ...
      {(convertibleTrainers ?? []).length > 0 && (
        ...
        >
          ...
          <select id="trainerId" name="trainerId" required defaultValue="" className="rounded border p-2">
      ...
      <form method="get" className="flex w-full max-w-2xl flex-col gap-2 rounded border p-3 text-sm">
        ...
        <input id="q" name="q" type="text" defaultValue={q ?? ''} className="rounded border px-3 py-2" />
      ...
      <div className="flex w-full max-w-2xl flex-col gap-2">
        {npcs.length === 0 ? (
          <p className="text-sm text-neutral-500">No NPCs match.</p>
        ) : (
          npcs.map((n) => (
            <Link key={n.id} href={`/trainers/${n.id}`} className="flex flex-col gap-1 rounded border p-3 hover:bg-neutral-50">
