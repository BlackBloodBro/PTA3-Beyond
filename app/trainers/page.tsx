...
import { deleteTrainer } from '@/app/trainers/actions'
  ...
  return (
    <main className="flex min-h-screen flex-col items-center gap-6 p-24">
      ...
      <div className="flex w-full max-w-2xl items-center justify-between">
        ...
        <Link href="/trainers/new" className="rounded bg-black px-4 py-2 text-sm text-white">
          + New trainer
        </Link>
      </div>
      ...
      <div className="flex w-full max-w-2xl flex-col gap-4">
        {(myTrainers ?? []).length === 0 ? (
          <p className="text-sm text-neutral-500">You don&apos;t have any trainers yet.</p>
        ) : (
          (myTrainers ?? []).map((t) => {
