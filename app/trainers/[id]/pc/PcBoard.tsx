      ...
      <div className="flex flex-1 flex-col gap-4">
        ...
        <form onSubmit={(e) => e.preventDefault()} className="flex flex-wrap items-end gap-2 rounded border p-3 text-sm">
          ...
          <div className="flex flex-col gap-1">
            ...
            <select id="pcType" value={typeId} onChange={(e) => setTypeId(e.target.value)} className="rounded border px-2 py-1">
        ...
        <div className="flex flex-col gap-2">
          {filteredPc.length === 0 ? (
            <p className="text-sm text-neutral-500">No Pokémon match.</p>
          ) : (
            filteredPc.map((p) => (
                ...
                {canManage &&
                  (pendingAdd?.id === p.id ? (
                    <div className="flex items-center gap-2 text-xs">
                      <span className="text-neutral-500">Pick who to bench in the Team sidebar →</span>
