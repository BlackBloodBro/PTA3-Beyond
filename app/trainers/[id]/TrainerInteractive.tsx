...
import { adjustTrainerHp, updateTrainerInfo, useFeatureCharge, resetFeatureUses } from '@/app/trainers/actions'
...
export function PendingMilestoneBanner({ trainerId }: { trainerId: string }) {
  ...
  return (
    <div className="flex w-full max-w-6xl items-center justify-between rounded border border-blue-600 bg-blue-50 p-4 text-blue-900">
      <span>Level {nextMilestoneLevel} unlocked a stat increase and advanced class choice.</span>
      <Link href={`/trainers/${trainerId}/level-up`} className="rounded bg-blue-600 px-3 py-1 text-sm text-white">
        Resolve now
      </Link>
    </div>
  )
      ...
      {isEditing ? (
        <div className="flex flex-col gap-3 text-sm">
          ...
          <div className="flex gap-2">
            <button type="button" onClick={handleSave} className="rounded bg-black px-3 py-1 text-white">
              Save
            </button>
              ...
              <ul className="ml-4 list-disc">
                {advancedClasses.map((ac) => (
                  <li key={ac.grantedAtLevel}>
                    {ac.name}
                    {canEdit && (
                      <Link href={`/trainers/${trainerId}/level-up/${ac.grantedAtLevel}`} className="ml-2 text-xs text-neutral-500 underline">
                        Edit
                      </Link>
                    )}
                  </li>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-2">
          <button
            ...
            className="rounded border border-green-600 px-3 py-2 text-sm font-semibold text-green-600"
          >
            Heal
          </button>
          <input
          ...
          <button
            type="button"
            onClick={() => handleAdjust(-1)}
            className="rounded border border-red-600 px-3 py-2 text-sm font-semibold text-red-600"
          >
            Damage
          </button>
        </div>
      ...
      {activeFeatures.length === 0 ? (
        ...
        <ul className="flex flex-col gap-2">
          {activeFeatures.map((f) => {
            ...
            return (
              <li key={f.id} className="rounded border border-amber-500 bg-amber-50 p-3">
                <details>
                  <summary className="cursor-pointer font-medium">
                    {f.name} <span className="rounded bg-amber-500 px-1.5 py-0.5 text-xs font-semibold text-white">ACTIVE</span>{' '}
                    <span className="text-sm font-normal text-neutral-500">(level {f.level_required})</span>
                  </summary>
        ...
        <ul className="flex flex-col gap-1">
          {passiveFeatures.map((f) => (
            <li key={f.id}>
              <details>
                <summary className="cursor-pointer font-medium">
                  {f.name} <span className="text-sm font-normal text-neutral-500">(level {f.level_required})</span>
                </summary>
                <p className="mt-1 pl-4 text-sm text-neutral-500">{f.description}</p>
              </details>
            </li>
          ))}
        </ul>
