...
import { resolveMilestone } from '@/app/trainers/actions'
...
export default async function LevelUpPage({
  ...
  const { data: trainer } = await supabase.from('trainers').select('name, level, class_id').eq('id', id).single()
      ...
      <form action={resolveMilestone.bind(null, id)} className="flex w-full max-w-2xl flex-col gap-4">
        <section className="rounded border p-4">
          ...
          <div className="flex gap-4">
            <select name="statA" className="rounded border p-2" required defaultValue="">
            ...
            <select name="statB" className="rounded border p-2" required defaultValue="">
              <option value="" disabled>
                Stat 2
              </option>
              {STAT_OPTIONS.map((s) => (
